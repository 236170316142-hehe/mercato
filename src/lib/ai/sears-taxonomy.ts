...
/** Full Sears path: "Category > Subcategory > Sub-Subcategory" */
export type SearsCategoryPath = string;

let cachedPaths: SearsCategoryPath[] | null = null;
let cachedPromptBlock: string | null = null;
let cachedMtime = 0;

function csvPath(): string {
  return join(process.cwd(), "src/lib/ai/data/sears_categories.csv");
}
...
/** Load and cache every leaf path from sears_categories.csv.
 *  Automatically reloads if the CSV file has been modified. */
export function loadSearsCategoryPaths(): SearsCategoryPath[] {
  ...
  if (paths.length === 0) {
    throw new Error("sears_categories.csv is empty or could not be parsed");
  }
...
export function clearSearsCache(): void {
  cachedPaths = null;
  cachedPromptBlock = null;
}

export function formatSearsTaxonomyForPrompt(): string {
  if (cachedPromptBlock) return cachedPromptBlock;

  const paths = loadSearsCategoryPaths();
